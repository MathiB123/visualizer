...
Une fonction $f(x)$ définie sur l'ensemble des réels est périodique si $f(x + T) = f(x)$ où $T$ est la période. Autrement dit, une fonction périodique se répète après un temps $T$. Par exemple, $\cos(x)$ et $\sin(x)$ sont des fonctions périodiques avec une période de $2\pi$. 
...
Une autre fréquence pertinente est la fréquence angulaire $\omega$, soit le nombre de radians parcourus par unité de temps. Pour la trouver, on peut dire que 
