...
Ici, la période vaut $T = N\tau$. En s'inspirant des fréquences discrètes d'une série de Fourier, on remplace $\omega$ par $\frac{2\pi n}{T} = \frac{2\pi n}{N\tau}$ dans la dernière équation pour discrétiser la fréquence. On trouve 

\begin{equation}
    c_n = \sum_{k=0}^{N-1}f(k\tau)e^{-i\frac{2\pi n}{N\tau}k\tau} = \sum_{k=0}^{N-1}f_ke^{-i\frac{2\pi}{N}nk} 
\end{equation}

Afin d'alléger l'écriture, on écrit que $f(k\tau) = f_k$ pour désigner le $k$ième point. Il reste à savoir les valeurs que peut prendre $n$. Comme hypothèse raisonnable supplémentaire, on peut dire que la fonction sous-jacente ne peut que contenir des fréquences à l'intérieur d'une certaine plage allant de 0 à $f_{\text{max}}$. Pour des raisons qu'on expliquera plus tard, si on pose $\tau = \frac{1}{f_x}$ où $f_x \geq f_{\text{max}}$, alors, dans la plage 0 à $f_x$, il y aura $\frac{f_x}{T} = N$ différentes fréquences qui sera suffisant pour représenter les fréquences présentes. Donc, $0 \leq n \leq N-1$. (4.2) correspond la TFD et on isole un des $f_k$ pour obtenir la transformée de Fourier inverse discrète (TFID).
