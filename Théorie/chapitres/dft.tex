...
Comme on l'a dit précédemment, dans la vraie vie, on enregistre une suite de $N$ points $\left\{f(t_k)\right\}_{k=0}^{N-1}$ à différents instants $t_k$. Par (4.1), on sait comment écrire la fonction échantillonnée. En la mettant dans (3.3), on calcule 
...
Puis, il n'est pas vraiment nécessaire de regarder toutes les fréquences $\omega$. Si on revient sur les séries de Fourier, on avait simplement besoin d'analyser une période pour avoir une bonne description des fréquences discrètes présentes dans la période en elle-même, donc aussi dans la fonction au complet de par sa nature périodique. Alors, pour la suite finie de mesures qu'on échantillonne, si on fait comme s'il s'agissait d'une fonction périodique (ce que la vraie fonction sous-jacente n'est pas nécessairement), des fréquences discrètes comme celles d'une série de Fourier sont amplement suffisantes. Ainsi, on aura la bonne description fréquentielle dans l'intervalle des mesures, mais sous le tapis la fonction sous-jacente est maintenant considérée comme périodique. Ici, la période vaut $T = N\tau$. On remplace $\omega$ par $\frac{2\pi n}{T} = \frac{2\pi n}{N\tau}$ dans la dernière équation pour discrétiser la fréquence. On trouve 

\begin{equation}
    c_n = \sum_{k=0}^{N-1}f(k\tau)e^{-i\frac{2\pi n}{N\tau}k\tau} = \sum_{k=0}^{N-1}f_ke^{-i\frac{2\pi}{N}nk} 
\end{equation}

Afin d'alléger l'écriture, on écrit que $f(k\tau) = f_k$ pour désigner le $k$ième point. Aussi, on note que $n$ est un entier se promenant entre 0 et $N-1$ par la périodicité de l'exponentielle complexe. Ainsi, (4.2) correspond la TFD et on isole un des $f_k$ pour obtenir la transformée de Fourier inverse discrète (TFID).
