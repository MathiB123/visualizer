...
Pour vérifier que cela tient, il faudrait évaluer l'intégrale, ce qui à priori ne semble pas simple du tout. Si on approximait l'intégrale (en prenant par exemple des bornes d'intégration qui tendent progressivement vers $-\infty/\infty$), on verrait graphiquement qu'on retombe bien sur la figure 6.
...
dont on peut montrer que l'intégrale redonne bien la fonction boîte.
...
\begin{equation*}
    c(\omega) = \lim_{k\rightarrow 0}\frac{1}{\pi k \omega}\sin(\frac{k\omega}{2}) = \frac{1}{2\pi} \lim_{k\rightarrow 0}\frac{2}{k\omega}\sin(\frac{k\omega}{2}) = \frac{1}{2\pi}\underbrace{\lim_{z\rightarrow 0}\frac{\sin(z)}{z}}_\text{1} = \frac{1}{2\pi}
\end{equation*}

où on utilise la substitution $z = \frac{k\omega}{2}$. Avec un différent choix de convention pour le facteur $\frac{1}{2\pi}$ dans l'équation de la TF/TFI (le mettre sur la TFI au lieu de la TF), le résultat donnerait 1, ce qui est plaisant.