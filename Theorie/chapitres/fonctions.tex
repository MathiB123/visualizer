...
\begin{figure}[h!]
    \centering
     \includegraphics[width=0.47\textwidth]{images/ch1/cos.png}
     \ \
...
Une fonction $f(x)$ définie sur l'ensemble des réels est périodique si $f(x + T) = f(x)$ où $T$ est la période. Autrement dit, une fonction périodique se répète après un temps $T$. Par exemple, $\cos(x)$ et $\sin(x)$ sont des fonctions périodiques avec une période de $2\pi$. D'ailleurs, il est aussi vrai que $f(x+nT) = f(x)$ pour tout entier $n$.

Une fonction périodique se caractérise aussi par sa fréquence $f$, soit le nombre de répétitions de la fonction par unité de temps. L'unité de fréquence est le hertz (Hz) dont 1Hz = 1s$^{-1}$ et les multiples d'une fréquence se nomment les harmoniques. On obtient la fréquence par $f = \frac{1}{T}$, puisque cela permet de calculer le nombre de périodes (donc le nombre de répétitions) dans une unité de temps. Par exemple, $\cos(x)$ et $\sin(x)$ ont les deux une fréquence de $\frac{1}{2\pi}$Hz. D'ailleurs, une autre fréquence pertinente est la fréquence angulaire $\omega$ qui correspond au nombre de radians parcourus par unité de temps. Pour la trouver, on peut dire que 

\begin{equation*}
    \omega = \frac{\text{rad}}{\text{s}} = \frac{\text{rad}}{\text{nb rép.}}\frac{\text{nb rép.}}{\text{s}} = 2\pi \cdot f = \frac{2\pi}{T}
\end{equation*}

car on fait un tour complet ($2\pi$ rad) à chaque cycle. Dès lors, les fonctions $\cos(x)$ et $\sin(x)$ ont une fréquence angulaire de 1 rad/s.
...
Alors, l'intégrale qui vient d'être dérivée vaut la même chose peu importe la valeur de $a$, ce qui donne (\ref{eq_int_sym}). Essentiellement, ce résultat nous dit que l'intégrale d'une fonction périodique est la même peu importe les bornes d'intégration tant que celles-ci sont séparées d'une période. Graphiquement, cela se voit en "déplaçant" un bout de l'intégrale.

\begin{figure}[h!]
    \centering
     \includegraphics[width=0.55\textwidth]{images/ch1/integrale_periode.jpg}
    \caption{Représentation graphique de l'équation (\ref{eq_int_sym})}
    \label{fig_int_sym}    
\end{figure}
