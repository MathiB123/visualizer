...
où on a utilisé la propropriété de linéarité, de déplacement en temps et de déplacement en fréquences de la TF. De plus, on utilise la série de Fourier du peigne de Dirac et la TF du delta de Dirac. On voit alors des copies de la TF du signal à 2Hz ($\pm 2$Hz) à chaque multiple de la fréquence d'échantillonnage $f_s = 16$, c'est-à-dire là où se trouvent les alias. 
...
Pour le moment, après avoir échantillonné un signal, la valeur de l'amplitude des échantillons correspond exactement à celle du signal à ces instants donnés. Pour un ordinateur, cela peut être problématique à traiter si l'amplitude contient un nombre infini de décimales par exemple. Ainsi, il faut aussi discrétiser les valeurs du signal analogique afin qu'un ordinateur puisse faire des calculs sur ces données. Naturellement, la quantification engendre une perte d'information irréparable contrairement à l'échantillonnage où, dans les bonnes conditions, la représentation du signal est parfaite.

Pour la quantification, il est possible de choisir des ints ou des floats avec plus ou moins de bits. Évidemment, plus le nombre de bits est grand, plus le nombre de valeurs distinctes que peuvent prendre ces ints/floats augmente. Cela permet de capturer de plus petites variations dans l'amplitude du signal et de faire en sorte que la quantification soit une très bonne approximation de l'amplitude du vrai signal. Cependant, la mémoire nécessaire sera conséquente et il peut être plus judicieux de diminuer le nombre de bits pour avoir des fichiers moins lourds.

\begin{figure}[H]
    \centering
        \includegraphics[width=0.3\textwidth]{images/ch6/quantification.png}
    \caption{Quantification d'un signal}
