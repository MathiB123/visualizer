...
Soient deux signaux sinusoïdaux analogiques à une fréquence de 2Hz et 18Hz respectivement qu'on échantillonne à $f_s = 16$Hz. On s'aide du delta de Dirac pour représenter mathématiquement l'échantillonnage des signaux.
...
Quel est l'impact de l'échantillonnage sur le spectre de fréquences du signal à 2Hz? Normalement, après avoir appliqué la TF, on s'attend à uniquement deux pics à $\pm$2Hz. Cependant, à cause de l'échantillonnage, 

\begin{equation*}
    \text{TF}\left(\sum_{m=-\infty}^{\infty}\sin(2\pi \cdot 2 \cdot \frac{m}{16})\delta\left(t - \frac{m}{16}\right)\right) = \text{TF}\left(\sin(2\pi \cdot 2t)\sum_{m = -\infty}^{\infty}\delta\left(t - \frac{m}{16}\right)\right)
\end{equation*}
\begin{equation*}
...
où on a utilisé la propriété de linéarité et de déplacement en fréquences de la TF ainsi que la série de Fourier du peigne de Dirac. On voit alors des copies de la TF du signal à 2Hz ($\pm 2$Hz) à chaque multiple de la fréquence d'échantillonnage $f_s = 16$, c'est-à-dire là où se trouvent les alias. 

\begin{figure}[H]
    \centering
        \includegraphics[width=0.5\textwidth]{images/ch6/aliasing_2hz.png}
    \caption{Effet de l'échantillonnage sur le spectre de fréquences du signal de 2Hz}
...
\begin{figure}[H]
    ...
    \caption{Effet de l'échantillonnage sur le spectre de fréquences d'un signal quelconque avec une plage de fréquences limitée}
...
\begin{figure}[H]
    ...
    \caption{Effet de l'échantillonnage sur le spectre de fréquences d'un signal quelconque avec une plage de fréquences limitée quand la fréquence d'échantillonnage n'est pas assez grande}
...
C'est ce qu'on appelle le théorème d'échantillonnage et, si respecté, il garantit que l'\textit{aliasing} ne causera aucun problème et que l'échantillonnage représente fidèlement le signal original dans la plage attendue de fréquences. De manière équivalente, on peut aussi dire que pour une fréquence d'échantillonnage $f_s$ donnée, la fréquence maximale qui ne subit pas les problèmes de l'\textit{aliasing} est $f_N = f_s / 2$ qu'on nomme la fréquence de Nyquist.

\subsubsection{Échantillonnage de l'audio}
Grosso modo, la plage de fréquences audibles pour les humains va de 20Hz à 20kHz. Donc, selon (\ref{eq_nyquist_shannon}), il faut que la fréquence d'échantillonnage d'un son soit supérieure à 40kHz. Par contre, en pratique, un micro va aussi enregistrer des fréquences bien au-delà de ce qu'on peut entendre et le choix de $f_s = 40$kHz ne sera pas suffisant pour éviter les problèmes dus à l'\textit{aliasing}. En fait, dans les convertisseurs analogique à numérique, qui font l'échantillonnage du son, il y a un filtre qui coupe les fréquences en dehors de la plage audible des humains avant de procéder à l'échantillonnage du signal. Pour des raisons techniques, le filtre ne peut pas couper nettement les fréquences pile au-delà de 20kHz et il restera quand même des fréquences, certes atténuées, un peu au-dessus de 20kHz avant qu'elles soient ensuite complètement bloquées. Alors, pour s'adapter à cette nouvelle plage, on doit prendre une fréquence d'échantillonnage un peu plus grande que 20kHz dont les standards sont 44.1kHz, 48kHz et bien d'autres selon les applications.  

\begin{figure}[H]
    \centering
        \includegraphics[width=0.55\textwidth]{images/ch6/filtre_echantillonnage.jpg}
    \caption{Filtre dans les convertisseurs analogique à numérique}
    \label{fig_filtre_echantillonage}
    \end{figure}

\subsection{Quantification}
Pour le moment, après avoir échantillonné un signal, la valeur de l'amplitude des échantillons correspond exactement à celle du signal à ces instants donnés. Pour un ordinateur, cela peut être problématique à traiter si l'amplitude contient un nombre infini de décimales par exemple. Ainsi, il faut discrétiser les valeurs du signal analogique afin qu'un ordinateur puisse faire des calculs sur ces données. Naturellement, la quantification engendre une perte d'information irréparable contrairement à l'échantillonnage où, dans les bonnes conditions, la représentation du signal est parfaite.

Pour la quantification, il est possible de choisir des \textit{ints} ou des \textit{floats} avec plus ou moins de bits. Évidemment, plus le nombre de bits est grand, plus ces \textit{ints} ou \textit{floats} peuvent prendre des valeurs distinctes. Cela permet de capturer de plus petites variations dans l'amplitude du signal et de faire en sorte que la quantification résulte en une très bonne approximation du vrai signal. Cependant, la mémoire nécessaire sera conséquente et il peut être plus judicieux de diminuer le nombre de bits pour avoir des fichiers moins lourds.

\begin{figure}[H]
    \centering
        \includegraphics[width=0.3\textwidth]{images/ch6/quantification.png}
    \caption{Quantification d'un signal sur 3 bits}
    \label{fig_quantification}
    \end{figure}